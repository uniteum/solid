...
1. **pool** - Call [pool()](https://etherscan.io/token/{{site.data.solids.1.address}}#readContract#F8){:target="_blank"} to get the current pool state (Solid balance, virtual ETH balance)
2. **buys** - Call [buys(ethAmount)](https://etherscan.io/token/{{site.data.solids.1.address}}#readContract#F3){:target="_blank"} to preview how many tokens you'd receive for a given ETH amount
3. **sells** - Call [sells(solidAmount)](https://etherscan.io/token/{{site.data.solids.1.address}}#readContract#F9){:target="_blank"} to preview how much ETH you'd receive for selling tokens
4. **sellsFor** - Call [sellsFor(solid, amount)](https://etherscan.io/token/{{site.data.solids.1.address}}#readContract#F10){:target="_blank"} to preview a Solid-to-Solid swap
5. **made** - Call [made(name, symbol)](https://etherscan.io/token/{{site.data.solids.NOTHING}}#readContract#F5){:target="_blank"} to check if a Solid exists and get its address
